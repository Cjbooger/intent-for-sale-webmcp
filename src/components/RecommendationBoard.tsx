...
export function RecommendationBoard({ offers, cleanOffers, phase, compared, onAudit, onStage }: { offers: OfferView[]; cleanOffers: OfferView[]; phase: string; compared: boolean; onAudit: (id: string) => void; onStage: (id: string) => void }) {
  ...
  return <Panel title="Decision review" eyebrow={compared ? "Market versus clean room" : "Commercial market"} action={<span className="header-action">{ranked.length ? `${ranked.length} offers` : "Awaiting auction"}</span>} className="recommendation-panel decision-review-panel">
