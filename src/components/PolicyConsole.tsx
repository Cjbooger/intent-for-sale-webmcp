...
export function PolicyConsole({ policy, phase, onApply, onPolicyChange }: { policy: PolicyView; phase: string; onApply: () => void; onPolicyChange: (policy: Partial<PolicyView>) => void }) {
  ...
  return <Panel title="Policy console" eyebrow={`Policy v${policy.sponsorshipMode === "block" ? "3.0" : "2.4"}`} action={<span className="policy-state">{policy.sponsorshipMode === "block" ? "CLEAN" : "OPEN"}</span>}>
